    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in xrange(self.epoch):
            ...
            if self.verbose > 0 and epoch%self.verbose == 0:
                tf.logging.info("Epoch %d [%.1f s]\ttrain=%.4f, validation=%.4f [%.1f s]"
        ...
        if self.pretrain_flag < 0 or self.pretrain_flag == 2:
            tf.logging.info("Save model to file as pretrain.")
...
def train(args):
    ...
    if args.verbose > 0:
        tf.logging.info("AFM: dataset=%s, factors=%s, attention=%d, freeze_fm=%d, #epoch=%d, batch=%d, lr=%.4f, lambda_attention=%.1e, keep=%s, optimizer=%s, batch_norm=%d, decay=%f, activation=%s"
    ...
    tf.logging.info("Best Iter(validation)= %d\t train = %.4f, valid = %.4f [%.1f s]"
...
def evaluate(args):
    ...
    tf.logging.info("Test RMSE: %.4f"%(RMSE))
    ...
    if args.mla:
        ...
        for i in range(3):
            _id = sorted_ids[i]
            tf.logging.info('## %d: %d'%(i+1, y_true[_id]))
            tf.logging.info('0.33*%.2f + 0.33*%.2f + 0.33*%.2f = %.2f'%(inter[_id][0], inter[_id][1], inter[_id][2], y_pred_fm[_id]))
            tf.logging.info('%.2f*%.2f + %.2f*%.2f + %.2f*%.2f = %.2f\n'%(\
